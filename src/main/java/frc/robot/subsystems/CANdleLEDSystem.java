...
public class CANdleLEDSystem extends SubsystemBase {
  ...
  private final StrobeAnimation green = new StrobeAnimation(0, 200, 200, 20, 0.6, 300);
  ...
  public void setAlignLED() {
    double maxAlignError = 3; // random value supposed to be max align error (if error is less than max, prog bar will start to fill up)
    double percentAlign = Math.min(1, Math.max(0, maxAlignError - alignError) / maxAlignError); // represents how aligned it is (1 = perfectly aligned , 0 = not even close)
    ...
    if (percentAlign > .9) {
      ...
      candle.setLEDs(0, 0, 0, 0, startLeft + (int)(len * percentAlign), endLeft - startLeft + (int)(len * percentAlign));
      candle.setLEDs(0, 0, 0, 0, startRight + (int)(len * percentAlign), endRight - startRight + (int)(len * percentAlign));
    }
  }
