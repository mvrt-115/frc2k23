...
public class MobileCharge extends CommandBase {
  ...
  @Override
  public void execute() {
    if(!onCharge && Math.abs(swerveDrivetrain.getPitchAngle()) > 3) {
      onCharge = true;
    }

    if(groundStart == -1 && onCharge && Math.abs(swerveDrivetrain.getPitchAngle()) < 3) {
      groundStart = Timer.getFPGATimestamp();
    }
    if(groundStart != -1 && Math.abs(swerveDrivetrain.getPitchAngle()) > 3)
      groundStart = -1;
  ...
  public boolean isFinished() {
    return (groundStart != -1 && Math.abs(groundStart - Timer.getFPGATimestamp()) > 0.75 && Math.abs(swerveDrivetrain.getPitchAngle()) < 2);
  }
}
